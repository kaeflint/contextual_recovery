...
def load_dataset(data_path: str,new_dataFormat=False):
    if not new_dataFormat:
        pack =  read_csv(data_path)
    else:
        pack = read_csv2(data_path)
    return pack
...
def load_all_data(dataset_path, mode="train",new_format_data=False):
    files = glob.glob(dataset_path + f"*{mode}.csv")
    print("processing files: ", files)
    dataset = []
    for file in files:
        dataset += load_dataset(file,new_dataFormat=new_format_data)
...
class ContextGenerationDatasetBoundary(Dataset):
    ...
    def __getitem__(self, index):
        return self.processTextsData(self.data[index])

    def change_data_mode(self, mode=1):
        self.mode = mode > 1
        
    def processTextsData(self,data: ContextualGenerationData):
        passage = data.input
        label_text = passage if self._is_auto_encoder_data else data.output
        # apply the tokenizer to convert the texts to the appropriate input
        if not self.mode:
            label_pack = self.tokenizer(
                label_text,
        ...
        boundary = eval(data.boundary_proportion)
        start_position = round(passage_seq.shape[0]* boundary[0])
        end_position = start_position + boundary[-1]
        
        if start_position > self.max_len:
            start_position = round(self.max_len*boundary[0])
            end_position = start_position + boundary[-1]
        
        if end_position > self.max_len:
            end_position = self.max_len
